...
public class DaoContentProv implements DaoManager
{
            ...
            pictureMgr)
    {
        ...
        {
            ContentProviderOperation.Builder insertPicPathBuilder = ContentProviderOperation
                    .newInsert(PicturesEntry.CONTENT_URI);
            ...
            insertPicPathBuilder.withValue(PicturesEntry.COLUMN_FILE_PATH, picture.getPicturePath
                    ());
            insertPicPathBuilder.withValue(PicturesEntry.COLUMN_LAST_UPDATED_ON, updateTime
                    .getTime());
        ...
        for (int j = 0; j < itemPrices.size(); ++j)
        {
            ...
            ContentValues priceContentValues = getPriceContentValues(price, itemId, updateTime,
                    null);
            ...
            if (buyItem.getSelectedPriceType() == price.getPriceType())
            {
                ...
                buyItemBuilder = buyItemBuilder.withValues(getBuyItemContentValues(buyItem,
                        updateTime))
                        .withValueBackReference(ToBuyItemsEntry.COLUMN_ITEM_ID, 0);

                buyItemBuilder = buyItemBuilder
                        .withValueBackReference(ToBuyItemsEntry.COLUMN_SELECTED_PRICE_ID, ops
                                .size() - 1);
        ...
        if (opSavePictureIdx > -1 && results != null)
        {
            ...
            {
                String deletePicMsg = cleanUpDiscardedPictures(results[opSavePictureIdx],
                        pictureMgr);
            ...
            pictureMgr)
    {
        ...
        for (int j = 0; j < itemPrices.size(); ++j)
        {
            ...
            ContentValues priceContentValues = getPriceContentValues(price, item.getId(),
                    updateTime, null);
        ...
        Uri updateBuyItemUri = ContentUris.withAppendedId(ToBuyItemsEntry.CONTENT_URI, buyItem
                .getId());
        ContentProviderOperation.Builder buyItemBuilder =
     ...
     */
    private int preparePictureForItemUpdateOps(PictureMgr pictureMgr,
                                               ArrayList<ContentProviderOperation> ops, Date
                                                       updateTime)
    {
        ...
        { //Update Item's picture operation
            Uri updatePictureUri = ContentUris.withAppendedId(PicturesEntry.CONTENT_URI,
                    originalPicture.getId());
            ContentProviderOperation.Builder updatePictureBuilder = ContentProviderOperation
                    .newUpdate(updatePictureUri);
            updatePictureBuilder.withValue(PicturesEntry.COLUMN_FILE_PATH, pictureLastViewed
                    .getPicturePath());
            updatePictureBuilder.withValue(PicturesEntry.COLUMN_LAST_UPDATED_ON, updateTime
                    .getTime());
        ...
        { //Insert Item's picture operation
            ...
            ContentProviderOperation.Builder insertPictureBuilder = ContentProviderOperation
                    .newInsert(insertPictureUri);
            insertPictureBuilder.withValue(PicturesEntry.COLUMN_FILE_PATH, pictureLastViewed
                    .getPicturePath());
            ...
            insertPictureBuilder.withValue(PicturesEntry.COLUMN_LAST_UPDATED_ON, updateTime
                    .getTime());
    ...
    public String update(Item item, List<Price> prices, PictureMgr pictureMgr)
    {
        ...
        Uri updateItemUri = ContentUris.withAppendedId(ItemsEntry.CONTENT_URI, item.getId());
        ContentProviderOperation.Builder updateItemBuilder = ContentProviderOperation.newUpdate
                (updateItemUri);
        ...
        {
            Uri updatePriceUri = ContentUris.withAppendedId(PricesEntry.CONTENT_URI, price.getId());
            ContentProviderOperation.Builder updatePriceBuilder = ContentProviderOperation
                    .newUpdate(updatePriceUri);
            updatePriceBuilder.withValues(getPriceContentValues(price, item.getId(), updateTime,
                    null));
        ...
        try
        {
            ...
            {
                String msgDiscardPics = cleanUpDiscardedPictures(results[opSavePictureIdx],
                        pictureMgr);
    ...
    private void logDbOperation(ContentProviderResult[] results)
    {
        ...
        for (int idx = 0; idx < results.length; ++idx)
        {
            if (idx == 0)
            {
                dbLog = "Saved : " + (results[idx].count != null ? results[idx].count :
                        results[idx].uri.toString());
            }
            else
            {
                dbLog += "\nSaved : " + ((results[idx].count != null) ? results[idx].count :
                        results[idx].uri.toString());
            }
        }
    ...
    public String delete(Item item, PictureMgr pictureMgr)
    {
        ...
        deletePriceBuilder.withSelection(PricesEntry.COLUMN_ITEM_ID + "=?", new String[]{String
                .valueOf(item.getId())});
        ...
        ContentProviderOperation.Builder itemDeleteBuilder = ContentProviderOperation.newDelete
                (uriDeleteItem);
        ...
        try
        {
            contentProviderResults = mContext.getContentResolver().applyBatch(Contract
                    .CONTENT_AUTHORITY, ops);
            msg = mContext.getString(R.string.database_success);
        } catch (RemoteException | OperationApplicationException e)
        {
            msg = mContext.getString(R.string.database_failed);
        ...
        msgCleanUp = cleanUpDiscardedPictures
                (contentProviderResults[deletePictureOpIndex], pictureMgr);
    ...
    private ContentValues getPriceContentValues(Price price, long itemId, Date updateTime,
    ...
    public String deleteCheckedItems()
    {
        ...
        int deleted = mContext.getContentResolver().delete(uriDeleteCheckedItem, where,
                selectionArgs);
